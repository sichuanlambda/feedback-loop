...
class AdminGrowthDashboardTest < ActionDispatch::IntegrationTest
  ...
  setup do
    # The page reads what the job cached; give the test a real store
  ...
  test "counts people, ignores crawlers, and attributes search traffic" do
    ...
    assert_select "[data-state=building]", 0
  ...
  test "shows a building state and queues one build until the report exists" do
    assert_enqueued_with(job: GrowthReportJob, args: [30, []]) { get admin_growth_path }
    assert_response :success
    assert_select "[data-state=building]", 1

    # A second load while the build is pending must not queue another
    assert_no_enqueued_jobs { get admin_growth_path }
  end
  ...
  test "internal users can be excluded without losing signed-out traffic" do
    ...
    GrowthReportJob.perform_now(30, [internal.id])
    get admin_growth_path
