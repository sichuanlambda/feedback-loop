...
class FeedbacksController < ApplicationController
  ...
  def ask_gpt
    ...
    if prompt_response
      ...
      # Create a new GptInteraction record and assign it to a variable
      gpt_interaction_record = GptInteraction.create(
      ...
      # Check if the GptInteraction record was successfully created
      if gpt_interaction_record.persisted?
        render json: { text: prompt_response, gpt_interaction_id: gpt_interaction_record.id }
      else
        render json: { error: 'Error creating GptInteraction record' }, status: :unprocessable_entity
      end
    else
